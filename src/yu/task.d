...
@trusted class AbstractTask
{
	...
	final bool job(){
		if(atomicLoad(_status) != TaskStatus.LDLE) return false;
		atomicStore(_status,TaskStatus.Runing);
		scope(failure) atomicStore(_status,TaskStatus.InVaild);
		bool rv = false;
		if(_runTask)
			rv = _runTask(this);
		if(rv)
...
@trusted auto makeTask(F,Alloc, Args...)(Alloc alloc,F delegateOrFp, Args args)
