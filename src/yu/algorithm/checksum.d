module yu.algorithm.checksum;

@nogc : 
// see . https://en.wikipedia.org/wiki/Fletcher%27s_checksum
ushort Fletcher16(const ubyte[] dt) @system
{
    ushort sum1 = 0xff, sum2 = 0xff;
    size_t tlen;
    size_t bytes = dt.length;
    ubyte * data = cast(ubyte *)dt.ptr;
    while (bytes) {
        tlen = ((bytes >= 20) ? 20 : bytes);
        bytes -= tlen;
        do {
            sum2 += sum1 += *data++;
            tlen--;
        } while (tlen);
        sum1 = (sum1 & 0xff) + (sum1 >> 8);
        sum2 = (sum2 & 0xff) + (sum2 >> 8);
    }
    /* Second reduction step to reduce sums to 8 bits */
    sum1 = (sum1 & 0xff) + (sum1 >> 8);
    sum2 = (sum2 & 0xff) + (sum2 >> 8);
    return cast(ushort)((sum2 << 8) | sum1);
}

// see . https://en.wikipedia.org/wiki/Fletcher%27s_checksum
uint Fletcher32(const ushort[] dt) @system
{
    uint sum1 = 0xffff, sum2 = 0xffff;
    size_t tlen;
    size_t words = dt.length;
    ushort * data = cast(ushort *)dt.ptr;
    while (words) {
        tlen = ((words >= 359) ? 359 : words);
        words -= tlen;
        do {
            sum2 += sum1 += *data++;
            tlen--;
        } while (tlen);
        sum1 = (sum1 & 0xffff) + (sum1 >> 16);
        sum2 = (sum2 & 0xffff) + (sum2 >> 16);
    }
    /* Second reduction step to reduce sums to 16 bits */
    sum1 = (sum1 & 0xffff) + (sum1 >> 16);
    sum2 = (sum2 & 0xffff) + (sum2 >> 16);
    return (sum2 << 16) | sum1;
}

// see : https://en.wikipedia.org/wiki/Adler-32
uint Adler32(ubyte[] data) 
{
    enum MOD_ADLER = 65521;
    uint a = 1, b = 0;
    /* Process each byte of the data in order */
    foreach(ch; data)
    {
        a = (a + ch) % MOD_ADLER;
        b = (b + a) % MOD_ADLER;
    }
    
    return (b << 16) | a;
}

//CRC(Cyclic Redundancy Check/循环冗余校验）
ushort CRC(ubyte[] data)
{
    enum ubyte[256] _auchCRCHi = [ ];
    // CRC低位字节值表
    enum ubyte[256]  _auchCRCLo = [ ] ;

    ubyte crcLo = 0xff; // 初始化校验结果低字节
    ubyte crcHi = 0xff; // 初始化校验结果高字节
    foreach(ch; data)
    {
        auto crcIndex = crcLo ^ ch;
        crcLo = cast(ubyte)(crcHi ^ _auchCRCHi[crcIndex]);
        crcHi = _auchCRCLo[crcIndex];
    }
    return cast(ushort)(crcHi << 8 | crcLo);
}

//Longitudinal Redundancy Check/纵向冗余校验
ubyte LRC(ubyte[] data)
{
    size_t rv = 0;
    foreach(ch; data)
        rv += ch;
    return cast(ubyte)((rv ^ 0xFF) + 1);
}

//Block Check Character
ubyte XOR(ubyte[] data)
{
    ubyte rv = 0;
    foreach(ch; data)
        rv ^= ch;
    return rv;
}
